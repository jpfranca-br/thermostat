...
void onSetpointChange() {
  printTimestamp();
  Serial.println(">>> Setpoint changed from Cloud Dashboard!");
...
void onModeChange() {
  printTimestamp();
  Serial.println(">>> Mode changed from Cloud Dashboard!");
  setLocalMode(mode);
}
...
void onHysteresisChange() {
  printTimestamp();
  Serial.println(">>> Hysteresis changed from Cloud Dashboard!");
  setLocalHysteresis(hysteresis);
}
...
void onProtectionTimeChange() {
  printTimestamp();
  Serial.println(">>> Protection Time changed from Cloud Dashboard!");
  setLocalProtectionTime(protectionTime);
}
...
void loadSettingsFromEEPROM() {
  if (EEPROM.read(0) == MAGIC_NUMBER) {
    EEPROM.get(1, localSetpoint);
    localMode = EEPROM.read(5);
    EEPROM.get(6, localHysteresis);
    EEPROM.get(10, localProtectionTime);
  } else {
    printTimestamp();
    Serial.println("First run: Initializing EEPROM with default settings.");
    localSetpoint = 4.0;
    localMode = false;
    localHysteresis = 1.0;
    localProtectionTime = 30;
    EEPROM.write(0, MAGIC_NUMBER);
    EEPROM.put(1, localSetpoint);
    EEPROM.write(5, localMode);
    EEPROM.put(6, localHysteresis);
    EEPROM.put(10, localProtectionTime);
    EEPROM.commit();
  }
}
 ...
 */
void printSettings() {
  printTimestamp(); Serial.println("--- Current Settings ---");
  printTimestamp(); Serial.print("--> Setpoint: "); Serial.println(localSetpoint);
  printTimestamp(); Serial.print("--> Mode: "); Serial.println(localMode ? "Hot" : "Cool");
  printTimestamp(); Serial.print("--> Hysteresis: "); Serial.println(localHysteresis);
  printTimestamp(); Serial.print("--> Protection Time: "); Serial.println(localProtectionTime);
  printTimestamp(); Serial.print("--> Protection state: "); Serial.println(protection ? "Active" : "Inactive");
  printTimestamp(); Serial.println("------------------------------------");
}
...
void syncLocalToCloud() {
  ...
  mode = localMode;
  hysteresis = localHysteresis;
  protectionTime = localProtectionTime;
}

/**
 * @brief Updates the local 'setpoint' variable and saves it to EEPROM.
 */
void setLocalSetpoint(float val) {
  localSetpoint = val;
  EEPROM.put(1, localSetpoint);
  EEPROM.commit();
  printSettings();
 ...
 */
void setLocalHysteresis(float val) {
  localHysteresis = val;
  EEPROM.put(6, localHysteresis);
  EEPROM.commit();
  printSettings();
 ...
 */
void setLocalProtectionTime(int val) {
  val = constrain(val, 0, 999);
  localProtectionTime = val;
  EEPROM.put(10, localProtectionTime);
  EEPROM.commit();
  printSettings();
}
 ...
 */
void runThermostatLogic(float tempC) {
  float lowerLimit = localSetpoint - localHysteresis;
  float upperLimit = localSetpoint + localHysteresis;
  
  if (localMode == true) { // Heating Mode
    if (tempC < lowerLimit) {
      setRelayState(true); // Temp is too cold, turn heater ON
    } else if (tempC > upperLimit) {
      setRelayState(false); // Temp is warm enough, turn heater OFF
    }
  } else { // Cooling Mode
    if (protection == false) { // CRITICAL: Only act if protection is OFF
      if (tempC > upperLimit) {
        setRelayState(true); // Temp is too hot, turn cooler ON
      } else if (tempC < lowerLimit) {
        setRelayState(false); // Temp is cool enough, turn cooler OFF
      }
    }
  }
}
...
void checkProtection() {
  if (localMode == true) {
    ...
    unsigned long protectionPeriod = (unsigned long)localProtectionTime * 1000UL;
    if (millis() - lastRelayChangeTime < protectionPeriod) {
      protection = true;
    } else {
      protection = false;
    }
  ...
  if (protection != lastProtectionState) {
    printTimestamp();
    Serial.print(">>> Protection status changed to: ");
    Serial.println(protection ? "Active" : "Inactive");
...
void setRelayState(bool newState) {
  if (relayState != newState) {
    ...
    if (relayState) {
      digitalWrite(RELAY_PIN, LOW); // For NC relay, LOW = ON
    } else {
      digitalWrite(RELAY_PIN, HIGH); // For NC relay, HIGH = OFF
    }
  }
}

/**
 * @brief Formats and displays a value on a 3-digit block of the 7-segment display.
 */
void formatThreeDigit(float value, int p1, int p2, int p3) {
  if (value >= 100 || value <= -100) {
    int v = abs((int)value);
    lc.setDigit(0, p1, (v / 100) % 10, 0);
    lc.setDigit(0, p2, (v / 10) % 10, 0);
    lc.setDigit(0, p3, v % 10, 0);
  } else if (value >= 0) {
    int t = (int)value / 10;
    if (t == 0) {
      lc.setChar(0, p1, ' ', 0);
    } else {
      lc.setDigit(0, p1, t, 0);
    }
    lc.setDigit(0, p2, (int)value % 10, 1);
    lc.setDigit(0, p3, (int)(value * 10) % 10, 0);
  } else if (value > -10) {
    float a = -value;
    lc.setChar(0, p1, '-', 0);
    lc.setDigit(0, p2, (int)a % 10, 1);
    lc.setDigit(0, p3, (int)(a * 10) % 10, 0);
  } else {
    int a = (int)-value;
    lc.setChar(0, p1, '-', 0);
    lc.setDigit(0, p2, (a / 10) % 10, 0);
    lc.setDigit(0, p3, a % 10, 0);
  }
}
...
void updateDisplay(float tempC) {
  lc.clearDisplay(0);
  char m = localMode ? 'H' : 'C';
  lc.setChar(0, 7, m, protection);
  formatThreeDigit(localSetpoint, 6, 5, 4);
  lc.setChar(0, 3, ' ', 0);
  formatThreeDigit(tempC, 2, 1, 0);
}
